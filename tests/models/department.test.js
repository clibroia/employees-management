...
describe("Department model", () => {
    ...
    it("should create a new department", async () => {
        ...
        assert.isTrue(Number.isInteger(savedDepartment.id), "id must be an integer")
        ...
        assert.strictEqual(savedDepartment.name, "Finance", "name must be strictly equal to Finance")
        assert.exists(savedDepartment.employeesNumber, "Department must have an employeesNumber field")
        assert.isTrue(Number.isInteger(savedDepartment.employeesNumber), "employeesNumber must be an integer")
        assert.strictEqual(savedDepartment.employeesNumber, 15, "employeesNumber must be strictly equal to 15")
        assert.exists(savedDepartment.budget, "Department must have a budget field")
        assert.isTrue(Number.isInteger(savedDepartment.budget), "budget must be an integer")
        assert.strictEqual(savedDepartment.budget, 1000000, "budget must be equal to 1,000,000")
        assert.exists(savedDepartment.email, "Department must have an email property")
        assert.isString(savedDepartment.email, "email must be a string")
        assert.match(savedDepartment.email, /^(?!.*\.\.)[^\s@]+[^.]@[^.][^\s@]+\.[^\s@]+$/, "email must match the prescribed regex")
        assert.strictEqual(savedDepartment.email, "good.department@example.com", "email must be strictly equal to good.department@example.com")
        assert.exists(savedDepartment.phone, "Department must have a phone property")
        assert.isString(savedDepartment.phone, "phone must be a string")
        assert.match(savedDepartment.phone, /^\+\d{7,15}$/, "phone must match the prescribed regex")
        assert.strictEqual(savedDepartment.phone, "+1234567890", "phone must be strictly equal to +1234567890")
    })
    ...
    it("should create a department with an employeesNumber value of zero when not provided", async () => {
        ...
        const savedDepartment = await department.save()
        assert.exists(savedDepartment.employeesNumber, "Department must records the number of employees")
        assert.isTrue(Number.isInteger(savedDepartment.employeesNumber), "employeesNumber must be an integer")
        assert.strictEqual(savedDepartment.employeesNumber, 0, "employeesNumber must be strictly equal to 0")
    })
    ...
    it("should create a department with a budget value of zero when not provided", async () => {
        ...
        assert.isTrue(Number.isInteger(savedDepartment.budget), "budget must be an integer")
        assert.strictEqual(savedDepartment.budget, 0, "budget must be strictly equal to 0")
    })
