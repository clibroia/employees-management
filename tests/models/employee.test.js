...
describe("Employee model", () => {
  ...
  it("should create a new employee", async () => {
    ...
    assert.isTrue(
      Number.isInteger(savedEmployee.id),
      "id property must be an integer",
    );
    assert.strictEqual(
      savedEmployee.id,
      1001,
      "id must be strictly equal to 1001",
    );
    assert.strictEqual(
      savedEmployee.name.firstName,
      "Alice",
      "firstName must be strictly equal to Alice",
    );
    assert.strictEqual(
      savedEmployee.name.middleName,
      "Marie",
      "middleName must be strictly equal to Marie",
    );
    assert.strictEqual(
      savedEmployee.name.lastName,
      "Johnson",
      "lastName must be strictly equal to Johnson",
    );
    assert.exists(savedEmployee.department.name, "department name is required");
    assert.isString(
      savedEmployee.department.name,
      "department name should be a string",
    );
    assert.strictEqual(
      savedEmployee.department.name,
      "HR",
      "department name must be strictly equal to HR",
    );
    ...
    assert.strictEqual(
      savedEmployee.role,
      "Manager",
      "role must be strictly equal to Manager",
    );
    assert.exists(savedEmployee.hiredOn, "Employee must have a hire date");
    assert.instanceOf(
      savedEmployee.hiredOn,
      Date,
      "hiredOn property must be an instance of Date",
    );
    assert.strictEqual(
      savedEmployee.hiredOn.toISOString().split("T")[0],
      "2020-01-15",
      "hiredOn property must be strictly equal to 2020-01-15",
    );
    ...
    assert.strictEqual(
      ...
      "email must be strictly equal to alice.johnson@example.com",
    );
    ...
    assert.strictEqual(
      savedEmployee.password,
      "$2b$10$4iHU2xivvDl7wIWjAeoX0.mP5s6RxNP5.WwMMlOYwvse13MaFllAG",
      "password must be strictly equal to $2b$10$4iHU2xivvDl7wIWjAeoX0.mP5s6RxNP5.WwMMlOYwvse13MaFllAG",
    );
  });
  ...
  it("should create a new employee without middle name", async () => {
    ...
    assert.strictEqual(
      savedEmployee.id,
      1002,
      "id must be strictly equal to 1002",
    );
    assert.strictEqual(
      savedEmployee.name.firstName,
      "Bob",
    ...
    assert.strictEqual(
      savedEmployee.hiredOn.toISOString().split("T")[0],
      "2018-06-01",
      "hiredOn property must be strictly equal to 2018-06-01",
    );
    assert.exists(savedEmployee.email, "Employee must have an email");
    assert.isString(savedEmployee.email, "email must be a string");
    assert.match(
    ...
    assert.strictEqual(
      ...
      "email must be strictly equal to bob.smith@example.com",
    );
    assert.exists(savedEmployee.phone, "Employee must have a phone number");
    assert.isString(savedEmployee.phone, "phone property must be a string");
    assert.match(
    ...
    assert.strictEqual(
      savedEmployee.phone,
      "+1987654321",
      "phone must be strictly equal to +1987654321",
    );
    assert.exists(savedEmployee.password, "Employee must have a password");
    assert.isString(
      savedEmployee.password,
    ...
    assert.strictEqual(
      savedEmployee.password,
      "$2b$10$IZa7r1OoiSHy.vpLr9B7vuys9UWW60.j1oXOpmJJJqxJHgvwMtbPe",
      "password must be strictly equal to $2b$10$IZa7r1OoiSHy.vpLr9B7vuys9UWW60.j1oXOpmJJJqxJHgvwMtbPe",
    );
  });
